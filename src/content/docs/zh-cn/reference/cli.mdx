...
Tauri 命令行接口(CLI) 是在整个开发生命周期中与Tauri进行交互的方式

你可以使用你的包管理器将 Tauri CLI 添加到你当前的项目中:
...
对于 CLI 命令相关的开发插件请访问 [插件开发指南](/zh-cn/develop/plugins/).
...
| [`dev`](#dev)                 | Tauri dev                         |
| [`icon`](#icon)               | 为所有主流平台生成各种 icon       |
| [`info`](#info)               | 展示关于Tauri的依赖和项目配置信息 |
| [`init`](#init)               | 初始化 Tauri 项目                 |
| [`add`](#add)                 | 管理 Tauri 插件                   |
| [`signer`](#signer)           | Tauri 更新器签名                  |
...
这个命令将在开发模式下打开 webview, 它将使用 `tauri.conf.json` 文件中的 `build.devPath` 属性.

如果你已经在 `build.beforeDevCommand` 属性中设置了一个命令, 那么这个命令将在 dev 命令之前执行.
...
`tauri info` 显示关于环境, rust, Node.js 机器版本以及一些相关项目配置的简明信息列表.
...
Tauri CLI 可以为 Bash, Zsh, PowerShell 和 Fish 生成shell自动完成脚本

以下是配置 Bash, Zsh, PowerShell的一些说明, 如果遇到问题, 请按照你的 shell 的官方说明操作.
请注意, 处于安全原因, 建议在执行生成的自动完成脚本之前先检查它的内容.

### Bash

获取 Bash 自动完成脚本并移动到一个已知的文件夹
